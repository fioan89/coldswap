...
/**
 * Finds any new method with this definition: public <any return type> <any method name>(long <any arg name>)
 * and insert them in the helper methods. If there are greater new methods than maximum of methods allowed, they
 * will be removed.
 */
public class PublicLongMethodReplacer implements MemberReplacer {
    private static final Logger logger = Logger.getLogger(PublicLongMethodReplacer.class.getName());
    ...
    public PublicLongMethodReplacer(Class<?> aClass, byte[] bytes, int maxMethods) {
        this.aClass = aClass;
        this.bytes = bytes;
                            ...
                            if (mParamType.length == 1 && mParamType[0].equals(Type.getType("J"))) {
                                if (counter < maxNumberOfMethods) {
                                    ...
                                    refManager.registerFieldReferenceReplacer(new PublicMethodReferenceReplacer(cn.name,
                                            mNode.name, mRetType, mParamType, Constants.LONG, counter));
                                    // autobox return type.
